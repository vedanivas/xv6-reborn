...
found:
  ...
  p->tkts = 1;
    ...
    p->entry_time = ticks;
    p->current_queue = 0;
    for (int i = 0; i < 5; i++) p->queue_ticks[i] = 0;
    p->waiting_limit[0] = 1;
    p->waiting_limit[1] = 2;
    p->waiting_limit[2] = 4;
    p->waiting_limit[3] = 8;
    p->waiting_limit[4] = 16;
  #endif
  return p;
}
...
int set_priority(uint64 priority, uint64 pid)
{
  ...
  for (p = proc; p < &proc[NPROC]; p++) {
      ...
      if (p->pid == pid) {
        ...
        int niceness = 5;
        if (p->rtime + p->stime != 0) niceness = (int)((p->stime / (p->rtime + p->stime)) * 10);
        ...
        if (dp_old > dp_new) yield();
        p->static_priority = dp_new;
        break;
      }
    #endif
  }
...
waitx(uint64 addr, uint* wtime, uint* rtime)
{
  ...
  fcfs(struct cpu *c)
  {
    ...
    for(struct proc *p = proc; p < &proc[NPROC]; p++) {
      acquire(&p->lock);
      if(p->state == RUNNABLE && (!minproc || p->ctime < mintime)) {
  ...
  /* End of code added */
	...
	*/
        ...
        if(ticket_count < golden_ticket)
	  {
        ...
        else if(ticket_count> total_tkts)
	  // printf("Extra: %d | %d | %d\n", ticket_count, total_tkts, golden_ticket);
    ...
    for (p = proc; p < &proc[NPROC]; p++) {
      if (p->state == RUNNABLE) {
        ...
        if (hpProc == 0) {
          ...
          if (hpProc->sched_times > p->sched_times) {
            hpProc = p;
          }
          else if (hpProc->sched_times == p->sched_times) {
...
mlfq(struct cpu *c)
{
    ...
    for (p = proc; p < &proc[NPROC]; p++) {
      if (p->state == RUNNABLE) {
        if ((ticks - p->entry_time > WAITING_LIMIT)) {
          acquire(&p->lock);
          p->queue_ticks[p->current_queue] = (ticks - p->entry_time);
          if(p->current_queue != 0) p->current_queue--;
          p->entry_time = ticks;
          release(&p->lock);
        }
      }
...
procdump(void)
{
  ...
  char *state;
  #if defined RR
    printf("\nPID\tState\trtime\twtime\tnruns");
  #endif
  
  #ifdef FCFS
    printf("\nPID\tState\trtime\tctime\tnruns");
  #endif
  #ifdef LBS
    printf("\nPID\tState\tTickets");
  #endif
  #ifdef PBS
    printf("\nPID\tPrio\tState\twtime\tctime\tnruns");
  #endif
  #ifdef MLFQ
    printf("\nPID\tQueue\tState\trtime\twtime\tnruns\tq0\tq1\tq2\tq3\tq4");
  #endif
  printf("\n");
  for(p = proc; p < &proc[NPROC]; p++){
    if(p->state == UNUSED)
    ...
    if(p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      if (end_time == 0)
        end_time = ticks;
      printf("%d\t%s\t%d\t%d\t%d", p->pid, state, p->rtime, end_time - p->ctime - p->rtime, p->sched_times);
      printf("\n");
    #endif
    #if defined FCFS
      int end_time = p->etime;
      if (end_time == 0)
        end_time = ticks;
      printf("%d\t%s\t%d\t%d\t%d", p->pid, state, p->rtime, p->ctime, p->sched_times);
      printf("\n");
    #endif
    
    #ifdef LBS
      printf("%d\t%s\t%d", p->pid, state, p->tkts);
      printf("\n");
    #endif
    #ifdef PBS
      int end_time = p->etime;
      ...
      int value = (p->static_priority - niceness + 5 < 100 ? p->static_priority - niceness + 5 : 100);
      int dp = (0 < value ? value : 0);
      printf("%d\t%d\t%s\t%d\t%d\t%d", p->pid, dp, state, end_time - p->ctime - p->rtime, p->ctime, p->sched_times);
      printf("\n");
    #endif
    #ifdef MLFQ
      int end_time = p->etime;
      ...
      if (p->state == ZOMBIE)
        current_queue = -1;
      printf("%d\t%d\t%s\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d", p->pid, current_queue, state, p->rtime, end_time - p->ctime - p->rtime, p->sched_times, p->queue_ticks[0], p->queue_ticks[1], p->queue_ticks[2], p->queue_ticks[3], p->queue_ticks[4]);
      printf("\n");
    #endif
  }
}
